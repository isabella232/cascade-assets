...
function Hammer(a, b, c) {
      ...
      return [
        {
          x:
            ...
            a.clientX +
              ...
              ((b && b.clientLeft) || (c && b.clientLeft) || 0),
          y:
            a.pageY ||
            a.clientY +
              ((b && b.scrollTop) || (c && c.scrollTop) || 0) -
              ((b && b.clientTop) || (c && b.clientTop) || 0),
        },
      ];
    } else {
      var d = [],
        e;
      for (var f = 0, g = a.touches.length; f < g; f++) {
        e = a.touches[f];
        d.push({ x: e.pageX, y: e.pageY });
      }
      return d;
    }
  }
  ...
  function H(c) {
    switch (c.type) {
      ...
      case "touchstart":
        ...
        var o = a.clientLeft || document.body.clientLeft || 0;
        var p = window.pageYOffset || a.scrollTop || document.body.scrollTop;
        var q = window.pageXOffset || a.scrollLeft || document.body.scrollLeft;
        r = { top: d.top + p - e, left: d.left + q - o };
        s = true;
        G.hold(c);
        if (b.prevent_default) {
          E(c);
        }
        break;
      case "mousemove":
      ...
      case "touchcancel":
      case "touchend":
        if (!s || (l != "transform" && c.touches && c.touches.length > 0)) {
          return false;
        }
        s = false;
        v = c;
        ...
        if (l == "drag") {
          ...
          D("transformend", {
            originalEvent: c,
            position: i.center,
            scale: B(i.start, i.move),
            rotation: C(i.start, i.move),
          });
        } else {
          G.tap(t);
        }
        m = l;
        D("release", { originalEvent: c, gesture: l });
        F();
        break;
    }
  }
  function I(b) {
    if (!J(a, b.relatedTarget)) {
      H(b);
    }
  }
  function J(a, b) {
    if (!b && window.event && window.event.toElement) {
      b = window.event.toElement;
    }
    if (a === b) {
      return true;
    }
  ...
  function M(a, b, c) {
    ...
    for (var d = 0, e = b.length; d < e; d++) {
      if (a.addEventListener) {
        a.addEventListener(b[d], c, false);
      } else if (document.attachEvent) {
        a.attachEvent("on" + b[d], c);
      }
    }
  }
  function N(a, b, c) {
  ...
  (function () {
    ...
    for (var f = 0; f < c.length; f++) {
      for (var g in d) {
        e = g;
        if (c[f]) {
          e = c[f] + e.substring(0, 1).toUpperCase() + e.substring(1);
        }
        a.style[e] = d[g];
      }
    }
  ...
  this.destroy = function () {
    if (w) {
      N(a, "touchstart touchmove touchend touchcancel", H);
    } else {
      N(a, "mouseup mousedown mousemove", H);
      N(a, "mouseout", I);
    }
  };
  var G = {
    hold: function (a) {
      if (b.hold) {
        ...
        q = setTimeout(function () {
          if (l == "hold") {
            D("hold", { originalEvent: a, position: i.start });
          }
        }, b.hold_timeout);
      }
    },
    swipe: function (a) {
      ...
      var e = i.move[0].y - i.start[0].y;
      f = Math.sqrt(c * c + e * e);
      var j = new Date().getTime();
      var k = j - n;
      if (b.swipe && b.swipe_time > k && f > b.swipe_min_distance) {
        g = z(i.start[0], i.move[0]);
        h = d.getDirectionFromAngle(g);
        l = "swipe";
        var m = { x: i.move[0].x - r.left, y: i.move[0].y - r.top };
        var o = {
          originalEvent: a,
          position: m,
          direction: h,
    ...
    drag: function (a) {
      ...
      if ((b.drag && f > b.drag_min_distance) || l == "drag") {
        ...
        var j = h == "up" || h == "down";
        if (
          ((j && !b.drag_vertical) || (!j && !b.drag_horizontal)) &&
          f > b.drag_min_distance
        ) {
          return;
        }
        l = "drag";
    ...
    transform: function (a) {
      if (b.transform) {
        ...
        var c = C(i.start, i.move);
        var d = B(i.start, i.move);
        if (
          l != "drag" &&
          (l == "transform" ||
            Math.abs(1 - d) > b.scale_treshold ||
            Math.abs(c) > b.rotation_treshold)
        ) {
          l = "transform";
          i.center = {
    ...
    tap: function (a) {
      ...
      var e = (function () {
        if (o && b.tap_double && m == "tap" && n - p < b.tap_max_interval) {
          ...
          var c = Math.abs(o[0].y - i.start[0].y);
      ...
      if (e) {
        l = "double_tap";
        p = null;
        D("doubletap", { originalEvent: a, position: i.start });
        E(a);
      } else {
        var g = i.move ? Math.abs(i.move[0].x - i.start[0].x) : 0;
        var h = i.move ? Math.abs(i.move[0].y - i.start[0].y) : 0;
        f = Math.max(g, h);
        if (f < b.tap_max_distance) {
          l = "tap";
          p = c;
          o = i.start;
          if (b.tap) {
            D("tap", { originalEvent: a, position: i.start });
            E(a);
          }
        }
      }
  ...
  if (w) {
    M(a, "touchstart touchmove touchend touchcancel", H);
  } else {
    M(a, "mouseup mousedown mousemove", H);
    M(a, "mouseout", I);
  }
}
    ...
    show: function () {
      ...
      if (n && this.enabled) {
        ...
        r.find(".tipsy-arrow")[0].className =
          "tipsy-arrow tipsy-arrow-" + u.charAt(0);
